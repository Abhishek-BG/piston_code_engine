...
app.post("/run-code", async (req, res) => {
  ...
  for (const input of inputs) {
    try {
      const result =
        language === "cpp"
          ? await piston.execute("cpp", code, {
              language: "cpp",
              version: "10.2.0",
              stdin: input,
            })
          : await piston.execute(language, code, { stdin: input });
      console.log(result);
...
app.post("/submit-code", async (req, res) => {
  ...
  console.log(language, code, inputs, correctLanguage, correctCode);
  let results = [];
  ...
  for (const input of inputs) {
    try {
      const correctResult =
        correctLanguage !== "cpp"
          ? await piston.execute(correctLanguage, correctCode, { stdin: input })
          : await piston.execute("cpp", correctCode, {
              language: "cpp",
              version: "10.2.0",
              stdin: input,
            });
      if (correctResult.run.stderr === "") {
      ...
      expectedOutputs.push(
        `Error: ${error.message || "Error executing correct code"}`
      );
    }
  }
